...
// Interfaces
interface VerifyXTwitterPageProps {}

/**
 * Verify XTwitter Page Component
 *
 * @interface VerifyXTwitterPageProps
 * @returns {JSX.Element} - VerifyXTwitterPage Component
 * @exports VerifyXTwitterPage
 */
export const VerifyXTwitterPage: FC<
  VerifyXTwitterPageProps
> = (): JSX.Element => {
  useDocumentTitle("AIScript - X/Twitter Verification");
  ...
  /**
   * Hook to check the X/Twitter OAuth response and redirect the user accordingly.
   */
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      if (params.has("error")) {
         ...
         * 1: The user must provide a valid X/Twitter profile to proceed with the login process.
         * 2: The email is already registered and is not a X/Twitter account.
         * 0: An error occured when trying to sign you in using X/Twitter.
         */
        switch (errorCode) {
          case 1:
            open?.({
              ...
              message:
                "You must provide a valid X/Twitter profile to proceed with the login process. Please try again.",
              key: "xtwitter_login_error",
            });
          ...
          case 2:
            open?.({
              ...
              description: "Something went wrong",
              message:
                "This email is already registered and is not a X/Twitter account. Please try logging in using your email and password.",
              key: "xtwitter_login_error",
            });
            navigate("/login", { replace: true });
            break;
          ...
          default:
            open?.({
              ...
              description: "Something went wrong",
              message:
                "An error occured when trying to sign you in using X/Twitter. Please try again.",
              key: "xtwitter_login_error",
            });
            navigate("/login", { replace: true });
            break;
  ...
  return (
    <>
      <Helmet>
        <title>AIScript - X/Twitter Verification</title>
        <meta
          name="description"
          content="Verify your X/Twitter account to proceed with the login process."
        />
      </Helmet>
